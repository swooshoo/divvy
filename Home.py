...
def get_user_inputs():
    ...
    with st.container(border=True):
        ...
        num_people = st.number_input("Divvied how many ways?", min_value=2, value=2, step=1)
        num_subtotal = st.number_input("Subtotal:", min_value=0.01, value=0.02, step=0.01, format="%0.2f")
...
def collect_item_totals(num_people):
    """Collects individual item totals from the user."""
    with st.container(border=True):
        st.subheader("Enter each person's item total:")
        return [st.number_input(f"Person {i+1} Item Total:", min_value=0.00, value=0.00, step=0.01, format="%0.2f") for i in range(num_people)]
...
def main():
    ...
    item_totals = collect_item_totals(num_people)
